...
main() {
    cd $DIR && \
	aws s3 cp s3://eplant-artifact/development/app-server/latest/eplant-server.zip . && \
	unzip -o eplant-server.zip && \
	rm eplant-server.zip && \
	mkdir -p config && \
	aws s3 cp s3://eplant-configuration/development/app-server/config/.env . && \
	npm install --loglevel=error && \
	npm run remotedev-start
}

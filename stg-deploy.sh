...
main() {
    IP_LIST=($(aws ec2 describe-instances \
		   --filters 'Name=tag:deployment-tag,Values=eplant-app-stg' \
		   --output text \
	...
	ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ubuntu@$ip '/home/ubuntu/eplant-server/stg-run.sh'
    done
}
